...
void MainWindow::on_registerButton_clicked()	{	
	...
	if(ui->pc1ToPc2Radio->isChecked())   {
		// need to clear the corrspondence list widget before scale, so scaled point will show up correctly
		ui->corrListWidget1->clear();
...
void MainWindow::on_corrClearButton1_clicked()	{
	pointCloud->getCorrPCContainer(POINTCLOUD1)->clear();
	ui->corrListWidget1->clear();

}

void MainWindow::on_corrClearButton2_clicked()	{
	pointCloud->getCorrPCContainer(POINTCLOUD2)->clear();
	ui->corrListWidget2->clear();
}
...
void MainWindow::pointCloudTransCallBack(std::string resultPath, pcl::PointCloud<pcl::PointXYZRGB>::Ptr srcCorrPoints, 
	...
	*/
	if(userChoice == QMessageBox::Yes)	{
		#ifdef DEBUGGING
			for(pcl::PointCloud<pcl::PointXYZRGB>::iterator it = pointCloud->getCorrPCContainer(POINTCLOUD1)->begin(); it != pointCloud->getCorrPCContainer(POINTCLOUD1)->end(); it++)	{
				std::cout << it->x << "  " << it->y << "  " << it->z << "  " << (unsigned int)it->r << "  " << (unsigned int)it->g << "  " << (unsigned int)it->b << "\n";
			}
			std::cout << "\n\n";
			for(pcl::PointCloud<pcl::PointXYZRGB>::iterator it = pointCloud->getCorrPCContainer(POINTCLOUD2)->begin(); it != pointCloud->getCorrPCContainer(POINTCLOUD2)->end(); it++)	{
				std::cout << it->x << "  " << it->y << "  " << it->z << "  " << (unsigned int)it->r << "  " << (unsigned int)it->g << "  " << (unsigned int)it->b << "\n";
			}
		#endif
		if(threadMode == THREAD_ENABLE)	{
